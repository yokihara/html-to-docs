...
- Free와 Pro를 작은 paste style 차이가 아니라 workflow capability 기준으로 분리합니다.
- MVP부터 extension UI를 영어와 한국어로 제공합니다.

## 비목표

- pixel-perfect HTML/CSS paste fidelity를 핵심 제품 약속으로 삼지 않습니다.
- `더 예쁜 paste`를 Pro의 primary differentiator로 삼지 않습니다.
...
Pro는 marginally prettier paste가 아니라 native workflow에 집중합니다.
...
Pro는 `Preserve report design`이 아니라 `Native Doc Mode` 또는 `Agent-to-Docs Workflow`로 마케팅합니다.
...
- 가능하면 MCP connection status
- Native Doc Mode가 locked 상태일 때 upgrade link
...
Extension은 사용자가 converter internals를 이해하지 않아도 되게 해야 합니다. Fallback path는 one button처럼, native path는 guided handoff처럼 느껴져야 합니다.
...
MVP에서 Native Doc Mode는 locked 또는 preview upgrade path로 보여줄 수 있지만, production billing에 의존하면 안 됩니다. 실제 checkout, account management, license enforcement는 이후 licensing phase에서 다룹니다. 그 전까지 Pro는 disabled control, waitlist/upgrade link, 또는 개발용 mock license flag로 표현할 수 있습니다.

첫 paywall은 native workflow capability입니다.
...
가장 큰 제품 리스크는 추출한 document intent가 실제로 유용한 native documentation을 만들 수 있는가입니다. Paste behavior는 fallback risk로 남지만, 더 이상 Pro의 핵심 약속은 아닙니다.
...
- source HTML, extracted document intent, Copy Clean output, Native Doc plan을 비교
- Copy Clean은 각 fixture를 Confluence test page에 paste하고 limitation 기록
- Native Doc Mode는 MCP 또는 mocked MCP payload로 test Confluence page를 create/update하고 결과 구조 리뷰
...
- native document outline이 source intent와 맞음
...
- Confluence 우선
- paste conversion은 fallback으로 취급하고 premium product promise로 삼지 않음
- Free는 Copy Clean fallback과 local intent extraction 제공
- Pro/Native Doc Mode는 Atlassian MCP 또는 native Confluence workflow로 차별화
